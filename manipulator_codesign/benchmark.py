...
class BenchMarkRobot:
    def __init__(self, robot_urdf_path: str, robot_home_pos, ik_tol=0.01, renders=True, ee_link_name='ee_link', ur5e=False):
        """
    ...
    def load_optimized_manip(self, target_poses, target_offset_poses):
        ...
        ch.build_robot()
        ch.save_urdf("/home/marcus/IMML/manipulator_codesign/manipulator_codesign/urdf/robots/temp.urdf")
        # elif self.ur5e:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ur5e", action="store_false", dest="ur5e", help="Run benchmark on ur5e")
    ...
    results = bench_mark.load_optimized_manip(
                target_poses=target_poses[len(target_poses) // 4 * 3 :], # use the last quarter of locations
                target_offset_poses=target_offset_poses[len(target_poses) // 4 * 3 :], # use the last quarter of locations
                )
